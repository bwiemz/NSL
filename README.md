...
The full reference — every subcommand, positional and flag, in the words of `nsl <command> --help` — is [`docs/wiki/CLI-Reference.md`](docs/wiki/CLI-Reference.md), generated by `nsl doc cli` and gated against the source. The short form:
...
- **Add a tensor op:** [crates/nsl-runtime/src/tensor/](crates/nsl-runtime/src/tensor/) — implement the op, add its row to the typed ABI table [crates/nsl-abi/src/table.rs](crates/nsl-abi/src/table.rs) (the codegen declares it from there and the runtime's build checks it), and the lowering under [crates/nsl-codegen/src/builtins/](crates/nsl-codegen/src/builtins/) if a program calls it by name
