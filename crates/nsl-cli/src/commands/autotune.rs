...
pub(crate) fn run_autotune(file: &Path, elements: usize, freeze: Option<&Path>) {
    if elements == 0 {
        nsl_log::nsl_log!(ERROR, "cli", "error: --elements must be at least 1 (0 elements benches nothing)");
        process::exit(1);
    }
    ...
    if analysis.diagnostics.iter().any(|d| d.level == nsl_errors::Level::Error) {
        ...
        nsl_log::nsl_log!(ERROR, "cli", "error: '{}' does not type-check", file.display());
        process::exit(1);
    }
        ...
        match measure(cap, &device, elements) {
            Ok(winner) => {
                ...
                match nsl_codegen::autotune::load_cache_record(&cap.name, &cap.hash, &device, None)
                {
                    ...
                    other => {
                        nsl_log::nsl_log!(ERROR, "autotune", 
            ...
            Err(e) => {
                nsl_log::nsl_log!(ERROR, "autotune", "[autotune] '{}': measurement failed: {e}", cap.name);
                failures += 1;
            }
        }
    }
    ...
    if let Some(out) = freeze {
        if frozen.is_empty() {
            nsl_log::nsl_log!(ERROR, "cli", "error: --freeze: no measured records to write");
            process::exit(1);
        }
        let (json, sha) = nsl_codegen::autotune::freeze_records(&mut frozen);
        if let Err(e) = std::fs::write(out, &json) {
            nsl_log::nsl_log!(ERROR, "cli", "error: --freeze {}: {e}", out.display());
            process::exit(1);
        }
        println!(
