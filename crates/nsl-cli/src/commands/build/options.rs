...
pub(crate) fn dispatch(args: crate::args::BuildArgs) {
    ...
    if let Some(ref db) = autotune_db {
        ...
        match nsl_codegen::autotune::load_frozen_db(db, autotune_db_sha256.as_deref()) {
            Ok(n) => nsl_log::nsl_log!(ERROR, "autotune", "[autotune] frozen db: {} record(s) from {}", n, db.display()),
            Err(e) => {
    ...
    let crate::meta_flags::PretrainBundle {
        ...
        fuse_lm_head,
            ...
            let wggo_memory_budget_bytes = match wggo_memory_budget {
                Some(0) => {
                    nsl_log::nsl_log!(ERROR, "cli", "error: --wggo-memory-budget must be > 0 MiB");
                    process::exit(1);
                }
            ...
            if autotune_clean {
                ...
                if cache_dir.exists() {
                    ...
                    nsl_log::nsl_log!(INFO, "nsl", "[nsl] autotune cache cleaned");
                } else {
                    nsl_log::nsl_log!(INFO, "nsl", "[nsl] no autotune cache to clean");
                }
                return;
            }
            ...
            if standalone {
                if weights.is_none() {
                    nsl_log::nsl_log!(ERROR, "cli", "error: --standalone requires -w/--weights <path>");
                    process::exit(1);
                }
