...
pub(crate) fn run_build_zk(
    ...
    if zk_proof_fns.is_empty() {
        nsl_log::nsl_log!(WARN, "zk", "[nsl/zk] no @zk_proof functions found — ZK circuit output skipped");
    } else {
        nsl_log::nsl_log!(INFO, "zk", 
            "[nsl/zk] found {} @zk_proof function(s):",
            zk_proof_fns.len()
        );

        if let Some(wpath) = zk_weights {
            nsl_log::nsl_log!(INFO, "zk", "[nsl/zk]   weights: {}", wpath.display());
        }
        ...
        for (fn_name, result) in &zk_results {
            ...
            if let Some(ref proof) = result.proof {
                ...
                if let Err(e) = std::fs::write(&proof_path, &proof.data) {
                    nsl_log::nsl_log!(ERROR, "zk", "[nsl/zk] error writing proof: {e}");
                } else {
                    nsl_log::nsl_log!(INFO, "zk", "[nsl/zk]   proof: {} ({} bytes, {} folds)",
                ...
                if let Err(e) = std::fs::write(&pi_path, &pi_json) {
                    nsl_log::nsl_log!(ERROR, "zk", "[nsl/zk] error writing public inputs: {e}");
                } else {
                    nsl_log::nsl_log!(INFO, "zk", "[nsl/zk]   public inputs: {}", pi_path.display());
                }
            }
        }
    }
    ...
    match cmd {
        crate::args::ZkCmd::Stats { file } => {
            match std::fs::read(&file) {
                Ok(data) => {
                    if data.len() < 12 {
                        nsl_log::nsl_log!(ERROR, "zk", "[nsl/zk] invalid proof file: too short ({} bytes)", data.len());
                        process::exit(1);
                    }
                ...
                Err(e) => {
                    nsl_log::nsl_log!(ERROR, "cli", "error reading {}: {e}", file.display());
                    process::exit(1);
                }
            }
        }
        crate::args::ZkCmd::Prove { file, pk: _, input: _, output } => {
            ...
            nsl_log::nsl_log!(INFO, "zk", "[nsl/zk] For the folding backend, proofs are generated during `nsl build --zk-circuit`.");
            nsl_log::nsl_log!(INFO, "zk", "[nsl/zk] The proof file is written alongside the binary as <file>.<fn_name>.proof");
            if let Some(ref out) = output {
                nsl_log::nsl_log!(INFO, "zk", "[nsl/zk] Requested output: {}", out.display());
            }
            nsl_log::nsl_log!(INFO, "zk", "[nsl/zk] To generate a proof, run: nsl build --zk-circuit {}", file.display());
        }
        crate::args::ZkCmd::Verify { vk: _, proof, public: _ } => {
            match std::fs::read(&proof) {
                Ok(data) => {
                    if data.len() < 12 {
                        nsl_log::nsl_log!(ERROR, "cli", "INVALID: proof file too short ({} bytes)", data.len());
                        process::exit(1);
                    }
                    ...
                    match M31Prover::verify(&zk_proof, &[]) {
                        ...
                        Err(e) => {
                            nsl_log::nsl_log!(ERROR, "cli", "VERIFICATION ERROR: {e}");
                            process::exit(1);
                        }
                    }
                }
                Err(e) => {
                    nsl_log::nsl_log!(ERROR, "cli", "error reading {}: {e}", proof.display());
                    process::exit(1);
                }
            }
        }
