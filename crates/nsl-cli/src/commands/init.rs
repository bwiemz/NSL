...
pub(crate) fn run_init(name: &str) {
    ...
    if root.exists() {
        nsl_log::nsl_log!(ERROR, "cli", "error: directory '{}' already exists", name);
        process::exit(1);
    }
    ...
    for dir in &[root.to_path_buf(), root.join("data"), root.join("weights")] {
        if let Err(e) = std::fs::create_dir_all(dir) {
            nsl_log::nsl_log!(ERROR, "cli", "error: could not create directory '{}': {e}", dir.display());
            process::exit(1);
        }
    }
...
";
    ...
    for (filename, contents) in files {
        ...
        if let Err(e) = std::fs::write(&path, contents) {
            nsl_log::nsl_log!(ERROR, "cli", "error: could not write '{}': {e}", path.display());
            process::exit(1);
        }
    }
