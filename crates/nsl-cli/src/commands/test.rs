...
pub(crate) fn run_test(file: &PathBuf, filter: Option<&str>) {
    ...
    if tests.is_empty() {
        if let Some(f) = filter {
            nsl_log::nsl_log!(ERROR, "cli", "no tests match filter '{f}'");
        } else {
            nsl_log::nsl_log!(ERROR, "cli", "no @test functions found");
        }
        process::exit(1);
    }
    ...
    for test_name in &tests {
        let output = std::process::Command::new(&exe_path)
            ...
            .output()
            .unwrap_or_else(|e| {
                nsl_log::nsl_log!(ERROR, "cli", "error: could not execute '{}': {e}", exe_path.display());
                process::exit(1);
            });
