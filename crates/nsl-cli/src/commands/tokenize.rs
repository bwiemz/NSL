...
pub(crate) fn run_tokenize(
    ...
    for dir in &search_dirs {
        ...
        if !dir_path.exists() {
            nsl_log::nsl_log!(WARN, "cli", "warning: directory '{}' not found, skipping", dir);
            continue;
        }
    ...
    if source_files.is_empty() {
        nsl_log::nsl_log!(ERROR, "cli", "error: no .{ext} files found in {:?}", search_dirs);
        process::exit(1);
    }

    nsl_log::nsl_log!(INFO, "tokenize", "[tokenize] Found {} .{} files across {} directories", source_files.len(), ext, search_dirs.len());
    ...
    let corpus_path = std::env::temp_dir().join("nsl_tokenizer_corpus.txt");
    {
        let mut corpus = std::fs::File::create(&corpus_path).unwrap_or_else(|e| {
            nsl_log::nsl_log!(ERROR, "cli", "error: could not create corpus file: {e}");
            process::exit(1);
        });
        ...
        for file in &source_files {
            match std::fs::read_to_string(file) {
                ...
                Err(e) => {
                    nsl_log::nsl_log!(WARN, "cli", "warning: could not read '{}': {e}", file.display());
                }
            }
        }
        nsl_log::nsl_log!(INFO, "tokenize", "[tokenize] Corpus: {} bytes from {} files", total_bytes, source_files.len());
    }
    ...
    let tokenizer = match nsl_runtime::tokenizer_bpe::train_from_file(
        ...
        Ok(t) => t,
        Err(e) => {
            nsl_log::nsl_log!(ERROR, "cli", "error: BPE training failed: {e}");
            process::exit(1);
        }
    };
    ...
    match tokenizer.save(output.to_string_lossy().as_ref(), true) {
        Ok(_) => {}
        Err(e) => {
            nsl_log::nsl_log!(ERROR, "cli", "error: could not save tokenizer to '{}': {e}", output.display());
            process::exit(1);
        }
    }

    let final_vocab = tokenizer.get_vocab_size(true);
    nsl_log::nsl_log!(INFO, "tokenize", "[tokenize] Saved tokenizer to '{}' (vocab_size={})", output.display(), final_vocab);
    ...
    if let Ok(encoding) = tokenizer.encode(sample, false) {
        ...
        nsl_log::nsl_log!(INFO, "tokenize", "[tokenize] Sample: \"{}\" -> {} tokens: {:?}", sample, tokens.len(), &tokens[..tokens.len().min(10)]);
    }
}
