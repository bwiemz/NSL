...
impl Default for HealthRenderer {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    pub fn format_block(&self, snap: &HealthSnapshot) -> String {
        ...
        if !snap.per_layer_grad_norm.is_empty() {
            ...
            for (idx, norm) in &layers {
                ...
                let tag = if **norm > GRAD_NORM_HEALTHY_MAX {
                    "⚠ elevated"
                } else {
                    "(healthy)"
                };
        ...
        if !snap.per_tensor_weight_pct_delta.is_empty() {
            ...
            for (label, entries) in group_by_layer_prefix(&snap.per_tensor_weight_pct_delta) {
                ...
                let parts: Vec<String> = entries
                    .iter()
                    .map(|(leaf, pct)| {
                        let mark = if pct.abs() > WEIGHT_DELTA_WARN_PCT {
                            " ⚠"
                        } else {
                            ""
                        };
        ...
        let nan_status = if snap.nan_inf_count_window == 0 {
            format!(
                "clean (0 NaN/Inf detected in {} steps)",
                snap.steps_in_window
            )
        } else {
            format!(
                "⚠ {} NaN/Inf detected in {} steps",
...
pub fn group_by_layer_prefix(items: &HashMap<String, f64>) -> Vec<(String, Vec<(String, f64)>)> {
    let mut groups: BTreeMap<(String, u32), Vec<(String, f64)>> = BTreeMap::new();
    for (path, value) in items {
        let segments: Vec<&str> = path.split('.').collect();
        let last_num_idx = segments
            .iter()
            .enumerate()
            .rev()
        ...
        match last_num_idx {
            Some(i) => {
                ...
                groups
                    .entry((label, layer_idx))
                    .or_default()
                    .push((leaf, *value));
            }
            None => {
                groups
                    .entry(("misc".to_string(), u32::MAX))
                    .or_default()
    ...
    groups
        .into_iter()
        .map(|((label, idx), v)| {
            let final_label = if idx == u32::MAX {
                "misc".to_string()
            } else if label == "L" {
                format!("L{}", idx)
            } else {
...
fn make_layer_label(prefix: &str) -> String {
    if prefix.contains("encoder") {
        "Enc.".into()
    } else if prefix.contains("decoder") {
        "Dec.".into()
    } else {
        "L".into()
    }
