...
fn run_export(file: &PathBuf, output: Option<&std::path::Path>, format: Option<&str>) {
    ...
    match mode.as_str() {
        "onnx" => {
            ...
            commands::build::run_run(file, &[], false, false, false, false, false, &nsl_codegen::CompileOptions::default());
        }
        "safetensors" => {
            if ext != "nslm" {
                eprintln!(
            ...
            match commands::convert::convert_nslm_to_safetensors(file, out_path) {
