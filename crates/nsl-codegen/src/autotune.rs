...
pub fn find_best_variant(
    ...
    match load_cache_record(kernel_name, cache_hash, device, None) {
        Ok(Some(rec)) if matches!(rec.selection, SelectionMethod::Measured) => {
            if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
                nsl_log::nsl_log!(INFO, "autotune", "[autotune] cache hit for {kernel_name} (measured)");
            }
            return Ok(rec.winner);
        }
        Ok(Some(_)) if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() => {
            nsl_log::nsl_log!(INFO, "autotune", 
    ...
    if std::env::var("NSL_AUTOTUNE_FALLBACK").is_ok() {
        if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
            nsl_log::nsl_log!(WARN, "autotune", "[autotune] fallback mode: picking median values (no GPU benchmarking)");
        }
    ...
    for variant in &all_variants {
        let ptx = match ptx_generator(variant) {
            ...
            Err(e) => {
                if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
                    nsl_log::nsl_log!(ERROR, "autotune", "[autotune]   {:?} => compile FAILED: {e}", variant);
                }
        ...
        match benchmark_fn(&ptx, kernel_name, variant) {
            ...
            Err(e) => {
                if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
                    nsl_log::nsl_log!(ERROR, "autotune", "[autotune]   {:?} => benchmark FAILED: {e}", variant);
                }
                continue;
            }
        }
...
pub fn find_best_variant_cost_model(
    ...
    if !fresh {
        ...
        {
            if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
                nsl_log::nsl_log!(INFO, "autotune", "[autotune] cache hit for {kernel_name}");
            }
            return Ok(cached);
        }
    } else if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
        nsl_log::nsl_log!(WARN, "autotune", "[autotune] --autotune-fresh: skipping cache for {kernel_name}");
    }

    // 2. Fallback mode
    if std::env::var("NSL_AUTOTUNE_FALLBACK").is_ok() {
        if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
            nsl_log::nsl_log!(WARN, "autotune", "[autotune] fallback mode: picking median values");
        }
        return Ok(select_middle_values(tuning_params));
    }
    ...
    for variant in &all_variants {
        ...
        match cost_estimator(variant) {
            ...
            Err(e) => {
                if std::env::var("NSL_AUTOTUNE_VERBOSE").is_ok() {
                    nsl_log::nsl_log!(ERROR, "autotune", "[autotune]   {:?} => cost estimate FAILED: {e}", variant);
                }
                continue;
            }
        }
...
pub fn print_benchmark_report(kernel_name: &str, results: &[BenchmarkResult]) {
    ...
    nsl_log::nsl_log!(INFO, "autotune", "{:-<72}", "");
    for r in results {
