...
fn plan_impl(
    ...
    if anchors.is_empty() {
        nsl_log::nsl_log!(WARN, "ccr", "[ccr] no block params are consumed; running without checkpointing");
        crate::pass_trace::record_disposition("CCR", PassDisposition::Declined {
    ...
    if recompute.is_empty() && compress.is_empty() {
        nsl_log::nsl_log!(WARN, "ccr", "[ccr] nothing recomputable found; running without checkpointing");
        crate::pass_trace::record_disposition("CCR", PassDisposition::Declined {
