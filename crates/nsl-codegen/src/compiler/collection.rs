    ...
    fn collect_strings_in_expr(&mut self, expr: &Expr) -> Result<(), CodegenError> {
        match &expr.kind {
            ...
            ExprKind::FString(parts) => {
                for part in parts {
                    match part {
                        nsl_ast::expr::FStringPart::Text(s) => {
                            self.intern_string(s)?;
                        }
                        nsl_ast::expr::FStringPart::Expr(e) => {
            ...
            ExprKind::DictLiteral(pairs) => {
                for (k, v) in pairs {
                    match &k.kind {
                        ...
                        _ => {
                            self.collect_strings_in_expr(k)?;
                        }
                    }
            ...
            ExprKind::Lambda { body, .. } => self.collect_strings_in_expr(body),
                ...
                for member in &md.members {
                    ...
                    } = member
                    {
                            ...
                            &type_ann.kind
                        {
                            ...
                            if let nsl_ast::types::TypeExprKind::Named(elem_sym) =
                                &element_type.kind
                            {
                                let elem_name = self.resolve_sym(*elem_sym).to_string();
                                field_type_map
                                    .insert(field_name, format!("[{};{}]", elem_name, size));
                            }
                            continue;
                        }
                    ...
                    } = member
                    {
                        for deco in decorators {
                            ...
                            if deco.name.len() == 1 && self.resolve_sym(deco.name[0]) == "paged_kv"
                            {
                                ...
                                let model_name = self.resolve_sym(md.name).to_string();
                                self.models.paged_kv_configs.insert(
                                    model_name,
                                    (num_blocks, block_size, num_heads, head_dim, num_layers),
                                );
                            }
                            ...
                            if deco.name.len() == 1
                                && self.resolve_sym(deco.name[0]) == "kv_compress"
                            {
                                ...
                                if let Some(ref args) = deco.args {
                                    for arg in args {
                                        if let Some(ref name_sym) = arg.name {
                                            ...
                                            match key.as_str() {
                                                "scheme" => {
                                                    if let nsl_ast::expr::ExprKind::IntLiteral(v) =
                                                        &arg.value.kind
                                                    {
                                                        scheme = *v;
                                                    }
                            ...
                            if deco.name.len() == 1 && self.resolve_sym(deco.name[0]) == "pipeline"
                            {
                                    ...
                                    for arg in args {
                                        if let Some(ref name_sym) = arg.name {
                                            ...
                                            match key.as_str() {
                                                ...
                                                "schedule" => {
                                                    if let nsl_ast::expr::ExprKind::StringLiteral(
                                                        s,
                                                    ) = &arg.value.kind
                                                    {
                                ...
                                self.features.pipeline_config =
                                    Some(crate::pipeline::PipelineConfig {
                                        num_stages: stages,
                                        schedule_type,
                                        checkpoint_stages,
                                    });
