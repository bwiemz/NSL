        ...
        if !allocs.is_empty() {
            ...
            if !plannable.is_empty() {
                ...
                if options.memory.report || plan.total_bytes > 0 {
                    ...
                    nsl_log::nsl_log!(INFO, "nsl", "[nsl] {}", report);
                }
            ...
            nsl_log::nsl_log!(INFO, "nsl", "[nsl] Memory plan: no static-shape tensor allocations found");
        }
    }
...
fn compile_with_zk_info_best_effort_plan(
    ...
    let pre_finalize = (|| -> Result<(), CodegenError> {
        ...
        if options.memory.report {
            nsl_log::nsl_log!(INFO, "nsl", "[nsl] --memory-report requested (planner integration in progress)");
        }
    ...
    for (fn_name, mode) in &zk_proof_fns {
        if let Some(fn_def) = compiler.features.zk_fn_defs.get(fn_name) {
            ...
            match crate::zk::compile_zk(fn_def, *mode, &zk_config, type_map, interner) {
                ...
                Err(e) => {
                    nsl_log::nsl_log!(WARN, "nsl", "[nsl] M55: ZK compilation warning for '{}': {}", fn_name, e);
                }
            }
        }
    }
