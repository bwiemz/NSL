...
impl<'a> Compiler<'a> {
    pub fn new(
        ...
        let call_conv = {
            ...
            if std::env::var("NSL_DEBUG").is_ok() {
                nsl_log::nsl_log!(INFO, "nsl", "[nsl] Cranelift ISA call convention: {:?}", detected);
            }
    ...
    pub fn record_ir(&self, label: std::fmt::Arguments<'_>, func: &cranelift_codegen::ir::Function) {
        ...
        if self.dump_ir {
            nsl_log::nsl_log!(INFO, "codegen", "--- IR: {label} ---\n{}", func.display());
        }
    ...
    pub(crate) fn finish_wgrad_admission(&self) -> Result<(), CodegenError> {
        ...
        if reasons.is_empty() {
            ...
            let reason = "this program has no train block, so the FASE-Deferred \
                          ...
                          is never built";
            nsl_log::nsl_log!(WARN, "wgrad-fusion", "[wgrad-fusion] declined: {reason}");
            reasons.push(reason);
            remedies.push(
