...
pub fn compile_wengert_ops_range(
    ...
    if compiler.compile_options.fusion.wgrad_accum && on_param_grad.is_some() {
        ...
        nsl_log::nsl_log!(INFO, "wgrad-fusion", 
        ...
        if !all_inputs_resolved {
            ...
            if is_live && !matches!(op.op, PrimalOp::FreeTensor) {
                ...
                if std::env::var("NSL_ALLOW_UNRESOLVED_LIVE_ADJOINT").as_deref() == Ok("1") {
                    nsl_log::nsl_log!(WARN, "source-ad", "[source-ad] WARNING (NSL_ALLOW_UNRESOLVED_LIVE_ADJOINT=1): {diag}");
                } else {
            ...
            if std::env::var("NSL_DEBUG_WENGERT").is_ok() {
                nsl_log::nsl_log!(WARN, "wengert-skip", 
...
fn emit_fused_forward_under_claim(
    ...
    let shmem_bytes_i64 = {
        ...
        for s in diags {
            nsl_log::nsl_log!(WARN, "codegen", "warning: {s}");
        }
        bytes
    };
...
fn emit_fused_lce_route_note(
    ...
    nsl_log::nsl_log!(INFO, "fused-lce", 
    ...
    if !use_gemm && is_large && dtype_tag != 0 {
        nsl_log::nsl_log!(WARN, "fused-lce", 
