...
fn lower_fused_linear_ce_forward(
    ...
    // sentinel.  An on-the-fly precision_cast plumbing pass (so the
    // wengert dispatch always allocates the 16-bit shadow buffers itself)
    // is the v6 ladder step.  v5 ships the plumbing-through-IR contract
    // tested by `fused_lm_ce_activation_end_to_end.rs`.
