...
// ---------------------------------------------------------------------------
// Analytical reference helpers — used by end_to_end_real_subprocess test
// ---------------------------------------------------------------------------

fn read_safetensors_flat(path: &std::path::Path, name: &str) -> Vec<f32> {
    let bytes = std::fs::read(path).unwrap_or_else(|e| panic!("read {:?}: {e}", path));
    let st = safetensors::SafeTensors::deserialize(&bytes).unwrap();
    let t = st.tensor(name).unwrap_or_else(|e| panic!("tensor {name}: {e}"));
    let data = t.data();
    assert!(data.len() % 4 == 0, "non-f32 tensor {name}");
...
/// Computes expected AWQ per-input-channel max-abs scales for the TinyMLP
/// fixture. Returns (up_proj_scales[64], down_proj_scales[128]).
///
/// calib is [8, 4, 64] row-major → 32 rows × 64 channels.
/// up_w is [128, 64] row-major (out=128, in_features=64).
fn reference_awq_scales(calib: &[f32], up_w: &[f32]) -> (Vec<f32>, Vec<f32>) {
    // up_proj input scales: max-abs over all 32 rows per channel (64 channels).
    let mut up = vec![0f32; 64];
    for r in 0..32 {
        for c in 0..64 {
            let v = calib[r * 64 + c].abs();
            if v > up[c] { up[c] = v; }
        }
    }
    // intermediate[r, o] = relu(sum_k calib[r, k] * up_w[o, k]) for r∈[0,32), o∈[0,128).
    let mut inter = vec![0f32; 32 * 128];
    for r in 0..32 {
        for o in 0..128 {
            let mut acc = 0f32;
            for k in 0..64 {
                acc += calib[r * 64 + k] * up_w[o * 64 + k];
            }
            inter[r * 128 + o] = if acc > 0.0 { acc } else { 0.0 };
        }
    }
    // down_proj input scales: max-abs over all 32 rows per channel (128 channels).
    let mut down = vec![0f32; 128];
    for r in 0..32 {
        for c in 0..128 {
            let v = inter[r * 128 + c].abs();
            if v > down[c] { down[c] = v; }
        }
    }
    (up, down)
}
...
fn assert_close(actual: &[f32], expected: &[f32], tol: f32, ctx: &str) {
    ...
    for (i, (&a, &e)) in actual.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(
            rel <= tol,
            "{ctx}: channel {i} actual={a} expected={e} rel_diff={rel}"
        );
    }
}

/// Parse AWQ scales out of a completed Sidecar.
fn sidecar_awq_scales(sidecar: &Sidecar) -> std::collections::HashMap<String, Vec<f32>> {
    let blob = sidecar
        .hooks
        .get("awq_activation_scales")
        .expect("sidecar must contain 'awq_activation_scales' key");
    let scales = nsl_runtime::awq::AwqScales::from_blob(blob)
        .expect("awq_activation_scales blob must parse");
    scales.by_projection
}

// ---------------------------------------------------------------------------
// End-to-end real-subprocess analytical reference test (Task 6)
// ---------------------------------------------------------------------------

/// Drives the complete real-subprocess calibration pipeline:
///   compile → link → spawn subprocess → sidecar read-back
/// Then verifies the AWQ per-input-channel max-abs scales match an
/// analytical reference computed in pure Rust f32 (no NSL matmul).
///
...
/// runs the same fabs/fmax loop so the dominant source of drift is the
/// model_forward matmul reordering, not the reduction.
#[test]
fn end_to_end_real_subprocess_matches_analytical_reference() {
    let nsl  = repo_root().join("tests/fixtures/awq_calibration_mlp.nsl");
    let data = repo_root().join("tests/fixtures/awq_calib_data.safetensors");
    let w    = repo_root().join("tests/fixtures/awq_calib_weights.safetensors");

    assert!(
        nsl.exists() && data.exists() && w.exists(),
        "fixtures present: nsl={} data={} w={}",
        nsl.exists(), data.exists(), w.exists()
    );

    // Ensure the stdlib_loader can find the SGD optimizer file.  The test
    // binary's cwd may differ from the repo root on some CI environments;
    // setting NSL_STDLIB_PATH explicitly to the worktree's stdlib directory
    // guarantees the train-block optimizer step is resolved at compile time.
    let stdlib_path = repo_root().join("stdlib");
    if stdlib_path.is_dir() {
        // Safety: safe in single-threaded test context.
        unsafe { std::env::set_var("NSL_STDLIB_PATH", &stdlib_path); }
    }

    let sidecar = nsl_codegen::compile_and_calibrate(&nsl, &data, &w)
        .expect("real-subprocess calibration pipeline runs");

    let calib = read_safetensors_flat(&data, "calibration");
    let up_w  = read_safetensors_flat(&w, "TinyMLP.up_proj");
    let (up_ref, down_ref) = reference_awq_scales(&calib, &up_w);

    let awq = sidecar_awq_scales(&sidecar);
    let up_actual   = awq.get("TinyMLP.up_proj").expect("up_proj in sidecar");
    let down_actual = awq.get("TinyMLP.down_proj").expect("down_proj in sidecar");
