...
fn fixture_zero_magnitude_row() {
    ...
    assert_eq!(q, vec![0i8; 8], "zero-magnitude row must produce all-zero quantized output");
}
...
fn fixture_uniform_magnitude_row() {
    ...
    let scale = absmax_scale_row(&row);
    assert!((scale - 1.5).abs() < 1e-6, "absmax of uniform 1.5 must be 1.5");
    let q = quantize_row_int8(&row, scale);
    // All values map to scale, so q = +127 across the row.
    assert_eq!(q, vec![127i8; 8], "uniform-positive row should clip to +127 across");
}
...
fn fixture_mixed_sign_mixed_magnitude_row() {
    ...
    assert_eq!(q, vec![76, -127, 25, -51], "mixed-sign quantization mismatch");
}
...
fn fixture_single_outlier_row() {
    ...
    assert_eq!(q, vec![1, 3, 127, 2], "outlier-row quantization mismatch (non-outliers compressed)");
}
