...
/// ```
///
/// After running, manually replace the `source_revision` placeholder in
/// the generated JSON with the current commit SHA (`git rev-parse HEAD`).
#[test]
...
fn generate_fixtures() {
    ...
    let file = FixtureFile {
        ...
        source_revision: String::from(
            "<run `git rev-parse HEAD` and paste current commit SHA>",
        ),
...
fn reference_matches_committed_fixtures() {
    ...
    for fixture in &file.fixtures {
        ...
        // Scales: bit-exact-ish (1e-6 rel / 1e-7 abs defensive tolerance).
        assert_eq!(
            scales.len(),
        ...
        // GEMM output: FP32 bit-exact-ish (1e-6 rel / 1e-7 abs defensive).
        assert_eq!(
            output.len(),
