...
fn emit_save_softmax_state_fires_hbm_save_under_save_activations_without_fused() {
    ...
    assert!(
        ...
        "HBM row_max save must fire under save_activations=true + fused_projections=false"
    );
    assert!(
        ptx.contains("row_sum_ptr"),
        "HBM row_sum save must fire same config"
    );
    // SMEM save portion should NOT fire (that's only for fused_projections=true S+PV split design)
    assert!(
        !ptx.contains("st.shared.f32 [%rd_wt_dst], %row_max"),
