...
// the CPKD fused KL-CE distillation kernels, exactly as `crates/nsl-codegen/
// src/cpkd_fused_loss.rs` held them at 16e1e42b, before the kernels moved
// onto KIR. Lines below this comment are that file's first 1077 lines (all
// but its unit tests), unedited; `cpkd_fused_loss_kir_equivalence.rs`
// includes it with `#[path]` and runs its output against the KIR kernels'.
// Do not change it: it is the pre-migration behaviour the equivalence
// claims are about.
//
