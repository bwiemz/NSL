...
//!   * `nsl_flash_attention_csha_backward`   — 53 params (+1 num_docs_or_zero as
//!     null sentinel; Sprint 5 will activate it when per-doc backward is wired)
//!
...
//!                       + num_docs_or_zero (null sentinel, Sprint 5 follow-on).
//!
...
fn csha_backward_decl_has_doc_starts_trailing_param() {
    ...
    //   + 1 segment_ids + 2 tier_b + 1 doc_starts + 1 num_docs_or_zero = 53.
    // num_docs_or_zero is passed as null today (Sprint 5 activates per-doc
    // CTA backward); builtins.rs and wengert_lower.rs are consistent at 53.
    assert_eq!(
        sig.params.len(),
        53,
        "nsl_flash_attention_csha_backward must accept 53 i64 params (PCA §4.3 Task 3 + Tier B + Sprint 5 sentinel)"
    );
}
