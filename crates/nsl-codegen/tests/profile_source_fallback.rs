...
fn pre_pass_reads_source_from_disk_when_text_is_none() {
    ...
    opts.dev_tools.profile_source_text = None;                                  // no explicit text
    opts.dev_tools.profile_source_file_name = Some(path.display().to_string()); // only path
...
fn pre_pass_prefers_explicit_text_over_disk_read() {
    let mut opts = CompileOptions::default();
    opts.dev_tools.profile_kernels = true;
    opts.dev_tools.profile_source_text = Some("# explicit override".to_string());
    opts.dev_tools.profile_source_file_name = Some("/nonexistent/bogus.nsl".to_string());
...
fn pre_pass_tolerates_missing_file_silently() {
    let mut opts = CompileOptions::default();
    opts.dev_tools.profile_kernels = true;
    opts.dev_tools.profile_source_text = None;
    opts.dev_tools.profile_source_file_name = Some("/definitely/not/a/real/path.nsl".to_string());
