...
pub struct Lexer<'a> {
    ...
    /// Stack of (brace_depth, quote_char, is_triple_quoted) for nested f-string expressions.
    fstring_stack: Vec<(u32, char, bool)>,
}
...
impl<'a> Lexer<'a> {
    ...
    fn scan_token(&mut self) {
        ...
        if !self.fstring_stack.is_empty() {
            if self.cursor.peek() == Some('}') {
                ...
                if let Some((depth, _, _)) = self.fstring_stack.last_mut() {
                    *depth += 1;
                }
    ...
    fn lex_fstring_text_with_quote(&mut self, quote: char, is_triple: bool) {
        let start = self.cursor.pos();
            ...
            match self.cursor.peek() {
                ...
                Some('{') => {
                    if self.cursor.peek_at(1) == Some('{') {
                        ...
                        // Push f-string state with brace depth 0 and the source quoting mode.
                        self.fstring_stack.push((0, quote, is_triple));
