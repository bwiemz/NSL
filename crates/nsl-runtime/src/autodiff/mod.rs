...
use crate::tensor::{nsl_tensor_free as tensor_free, NslTensor};
...
impl TapeOp {
    fn assign_ids(&mut self, tape: &mut Tape) {
        match self {
            ...
            | TapeOp::Mul { a, b, out, .. }
            | TapeOp::Div { a, b, out, .. }
            | TapeOp::MatMul { a, b, out, .. }
            | TapeOp::Fp8MatMul { a, b, out, .. } => {
                *a = tape.get_or_assign_id(*a);
                *b = tape.get_or_assign_id(*b);
                *out = tape.get_or_assign_id(*out);
            }
            TapeOp::Neg { a, out }
            | TapeOp::MulScalar { a, out, .. }
            | TapeOp::AddScalar { a, out }
            | TapeOp::Transpose { a, out, .. }
            | TapeOp::SumReduce { a, out, .. }
            | TapeOp::MeanReduce { a, out, .. }
            | TapeOp::ReduceMax { a, out, .. }
            | TapeOp::Gather { a, out, .. }
            | TapeOp::Exp { a, out, .. }
            | TapeOp::Log { a, out, .. }
            | TapeOp::Sqrt { a, out, .. }
            ...
            | TapeOp::Tanh { a, out, .. }
            | TapeOp::Softmax { a, out, .. }
            | TapeOp::LogSoftmax { a, out, .. }
            | TapeOp::Slice { a, out, .. }
            | TapeOp::Dropout { a, out, .. }
            | TapeOp::RotateHalf { a, out, .. }
            | TapeOp::MaxPool2d { a, out, .. } => {
                *a = tape.get_or_assign_id(*a);
                *out = tape.get_or_assign_id(*out);
            }
            TapeOp::EmbeddingLookup {
                ...
                *weight = tape.get_or_assign_id(*weight);
                *indices = tape.get_or_assign_id(*indices);
                *out = tape.get_or_assign_id(*out);
            }
            TapeOp::LayerNorm {
                input,
            ...
            | TapeOp::Conv2d {
                ...
                *input = tape.get_or_assign_id(*input);
                *weight = tape.get_or_assign_id(*weight);
                *bias = tape.get_or_assign_id(*bias);
                *out = tape.get_or_assign_id(*out);
            }
            TapeOp::RMSNorm {
                input, weight, out, ..
            ...
            TapeOp::BiasAdd { tensor, bias, out } => {
                *tensor = tape.get_or_assign_id(*tensor);
                *bias = tape.get_or_assign_id(*bias);
                *out = tape.get_or_assign_id(*out);
            }
            ...
            TapeOp::Cat { inputs, out, .. } | TapeOp::Stack { inputs, out, .. } => {
                for inp in inputs.iter_mut() {
                    *inp = tape.get_or_assign_id(*inp);
                }
                *out = tape.get_or_assign_id(*out);
            }
            TapeOp::FlashAttention { q, k, v, .. } => {
                *q = tape.get_or_assign_id(*q);
                *k = tape.get_or_assign_id(*k);
                *v = tape.get_or_assign_id(*v);
                // out stays raw pointer — backward reads tape_id from tensor
            }
            TapeOp::Checkpoint { output, .. } => {
                *output = tape.get_or_assign_id(*output);
            }
        }
    }
}
...
pub(crate) fn call_with_args(fn_ptr: usize, args: &[i64]) -> i64 {
    debug_assert_eq!(
        ...
        8,
        "gradient checkpointing requires 64-bit target"
    );
    ...
    unsafe {
        match args.len() {
            ...
            5 => std::mem::transmute::<usize, Fn5>(fn_ptr)(
                args[0], args[1], args[2], args[3], args[4],
            ),
            6 => std::mem::transmute::<usize, Fn6>(fn_ptr)(
                args[0], args[1], args[2], args[3], args[4], args[5],
            ),
            7 => std::mem::transmute::<usize, Fn7>(fn_ptr)(
                args[0], args[1], args[2], args[3], args[4], args[5], args[6],
            ),
            8 => std::mem::transmute::<usize, Fn8>(fn_ptr)(
...
#[no_mangle]
pub extern "C" fn nsl_checkpoint_record(
    fn_ptr: i64,
    args_list: i64,
    tensor_mask: i64,
    output: i64,
