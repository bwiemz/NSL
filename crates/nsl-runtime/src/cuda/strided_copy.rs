...
/// The module holding the four arms, selected by [`RunPlan::kernel_name`].
///
/// The kernels are described as KIR in `nsl_kir::kernels::strided_copy`
/// (roadmap A2 step 11), which documents their common contract: `(src, dst,
/// offsets, run_len, outer)`, `grid.x` spanning one run and `grid.y` the
/// runs (grid-stride, so `outer` may exceed 65535), run `o` of `dst` at
/// element `o * run_len`. They replace a hand-written module of the same
/// four entries; `nsl-codegen`'s `strided_copy_kir_equivalence` gate holds
/// the two to the same bytes.
///
/// Built on first use and kept: `kernel_launch` keys its module cache on
/// the buffer's address, so every launch must pass the same one. The text
/// is NUL-terminated ASCII, as `cuModuleLoadData` requires.
pub(crate) fn run_module() -> &'static str {
    ...
    MODULE.get_or_init(|| {
        ...
        // `nsl-kir` builds.
        String::from_utf8(nsl_kir::kernels::strided_copy::ptx()).expect("PTX must be ASCII")
    })
}
...
mod tests {
    ...
    fn geometry_covers_every_unit_and_respects_grid_limits() {
        for (shape, src) in [
            ...
            // The kernels declare this bound as their `.maxntid`.
            assert!(block[0] <= nsl_kir::kernels::strided_copy::SCOPY_MAX_BLOCK as i64);
    ...
    fn ptx_is_pure_ascii_and_nul_terminated() {
        ...
        assert!(!ptx.contains("mad.lo.u32"), "mad.lo.u32 is not valid in PTX ISA 7.0");
        // One build, one address: the module cache keys on it.
        assert!(std::ptr::eq(ptx, run_module()));
    }
    ...
    fn every_plan_arm_has_an_entry_point() {
        for (broadcast, vec4) in [(false, false), (false, true), (true, false), (true, true)] {
            ...
            assert!(
                run_module().contains(&format!(".visible .entry {name}(")),
                "{name} has no entry point in the module"
            );
            // `arm_index` and the kernel description agree on which arm is
            // which.
            let arm = nsl_kir::kernels::strided_copy::ScopyArm::ALL[plan.arm_index()];
            assert_eq!(arm.kernel_name(), name);
            assert_eq!((arm.broadcast(), arm.vec4()), (broadcast, vec4));
        }
    }
}
