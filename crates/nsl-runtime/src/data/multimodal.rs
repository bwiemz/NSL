    ...
    match tag {
        ...
        TAG_VIDEO => {
            ...
            for _ in 0..num_frames {
                ...
                let frame_len =
                    u32::from_le_bytes(rest[offset..offset + 4].try_into().ok()?) as usize;
                offset += 4;
                if offset + frame_len > rest.len() {
                    return None;
                }
...
pub fn group_by_modality(samples: Vec<Sample>) -> std::collections::HashMap<Modality, Vec<Sample>> {
    let mut groups: std::collections::HashMap<Modality, Vec<Sample>> =
...
mod tests {
    ...
    fn test_malformed_image_data_len() {
        ...
        assert!(
            deserialize_sample(&buf).is_none(),
            "should reject oversized data_len"
        );
    }
}
