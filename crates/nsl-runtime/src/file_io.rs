...
pub extern "C" fn nsl_read_file(path: i64) -> i64 {
    ...
    match std::fs::read(path_str) {
        ...
        Err(_) => {
            crate::nsl_log!(ERROR, "nsl", "nsl: could not open file '{}' for reading", path_str);
            std::process::abort();
        }
    }
}
...
pub extern "C" fn nsl_write_file(path: i64, content: i64) {
    ...
    if std::fs::write(path_str, content_str).is_err() {
        crate::nsl_log!(ERROR, "nsl", "nsl: could not open file '{}' for writing", path_str);
        std::process::abort();
    }
}
...
pub extern "C" fn nsl_append_file(path: i64, content: i64) {
    ...
    let mut file = match std::fs::OpenOptions::new().append(true).create(true).open(path_str) {
        Ok(f) => f,
        Err(_) => {
            crate::nsl_log!(ERROR, "nsl", "nsl: could not open file '{}' for appending", path_str);
            std::process::abort();
        }
    };
    if file.write_all(content_str.as_bytes()).is_err() {
        crate::nsl_log!(ERROR, "nsl", "nsl: could not write to file '{}'", path_str);
        std::process::abort();
    }
}
