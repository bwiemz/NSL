    ...
    for b in 0..batch {
        for h in 0..heads {
            for qi in 0..seq {
                ...
                for (ki, s_val) in s_row.iter_mut().enumerate().take(k_limit) {
                ...
                let mut rsum = 0.0f32;
                for s_val in s_row.iter().copied().take(k_limit) {
                    rsum += (s_val - rmax).exp();
                }
                ...
                for di in 0..hd {
                    let mut acc = 0.0f32;
                    for (ki, s_val) in s_row.iter().copied().enumerate().take(k_limit) {
                        let p = (s_val - rmax).exp() / rsum;
                        let k_base = ((b * heads + h) * seq + ki) * hd;
