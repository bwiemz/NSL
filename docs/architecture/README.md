...
| [`codegen.md`](codegen.md) | `nsl-codegen` and `nsl-kir` — entry points and `CompileOptions`, the `Compiler` driver and the train-block compiler, the runtime-ABI registry, KernelIR (the `nsl-kir` leaf crate) vs hand-written PTX, autodiff, the pass scheduler, the error model |
| [`runtime.md`](runtime.md) | `nsl-runtime` (and `nsl-log`, the logging front door) — the tensor handle model, memory, the autodiff tape, the CUDA backend, training state and checkpoints, data, serving and interop, observability |
