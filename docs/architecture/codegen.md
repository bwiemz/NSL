...
The struct has 87 `pub` fields today. The decomposition into cohesive
...
`CpdtOptions` (`opts.cpdt`), plus `MatmulConfig` (`opts.matmul`) and
...
`vram_budget`, `memory_report`, `calibration_*`, …). New options belong in a
sub-struct when they share a subsystem; otherwise a flat field is acceptable
but should carry a doc comment naming the flag.
...
1. Add the field to `CompileOptions` in `src/lib.rs` — inside the matching
   ...
   `CshaOptions`, `CpdtOptions`, `MatmulConfig`) when one exists — with its
