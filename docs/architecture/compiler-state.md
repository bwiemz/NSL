...
| `nsl-runtime/src/transient_arena.rs` | `CPU_ONLY_PIN` | TEST | The NON-cuda build's stand-in for the arena pin, which in a cuda build is a field of the `THREAD_SLOTS` slot (roadmap A4 step 4b). Without the cuda feature nothing allocates device memory, so no production path ever consumes a pin — the region is never set and bind returns before arming; the cell exists so `take_pin`'s exact-size gate runs under plain `cargo test -p nsl-runtime`, the same reason step 3c kept a process-global `CPU_ONLY` region for that build. Declared inside `with_pin`, the only door to it. |
| `nsl-runtime/src/tensor/mod.rs` | `OFFLOAD_DRAIN_TENSORS`, `OFFLOAD_DRAIN_DEVICE_BUFS` | RUNTIME-OK | In-flight async DtoH copy-back queues, deliberately paired with the transfer stream in `THREAD_SLOTS` — converting to shared state would be a regression. They move into the slot alongside it when a step needs them device-keyed; today they key only work the calling thread issued, which the slot's transfer stream already scopes. |
...
| `nsl-runtime/src/cuda/context.rs` | `THREAD_SLOTS` | RUNTIME-OK | Roadmap A4 step 2: ONE cell replacing eight — the three named CUDA streams (`COMPUTE_STREAM`, `TRANSFER_STREAM`, `INSPECT_STREAM`) and the five device-pointer workspaces (`WS`, `SR_WS`, `CE_SCRATCH`, `MUON_STATS_BUF`, `muon_batch`'s `WS_CACHE`), now held per (thread, DEVICE) and indexed by registry slot. Thread-local is still the correct ownership model and is load-bearing, not incidental: the compute stream is a BLOCKING stream, two blocking streams do not synchronize with each other, so only the thread that launched work may record ordering events against it. What the move buys is the device half — a second device gets its own streams and scratch instead of silently sharing device 0's, which is what step 5 needs. Workspaces are keyed by `TypeId` so each stays declared where it is used. Step 4 folds in four more — `graph_capture`'s `ACTIVE`, `REGIONS`, `OCCURRENCE`, `NESTED_SKIP` — as the slot's `CaptureState` field: a region captures on the calling thread's compute stream, so the state machine is thread-affine for the same reason the stream is, and a captured `CUgraphExec` belongs to one context, so it is device-affine too. Step 4b folds in the allocator's placement channel — `caching_allocator`'s `CURRENT_POOL` and the transient arena's `PIN` / `PLACED_AT` — as the slot's `Placement` field: both steer the NEXT allocation the calling thread makes, from a caller that cannot pass it down an `extern "C"` row, so they are thread-affine by construction. Their door, `context::with_placement`, is the one slot door that creates no context, because `nsl_gpu_set_*_pool` and `nsl_arena_unbind` run in CPU-only programs too. |
...
3. **Capture/allocator state machines**: done. Roadmap A4 step 4 moved
   `graph_capture`'s four cells into `THREAD_SLOTS` (4a), then
   `CURRENT_POOL` and the transient arena's `PIN` / `PLACED_AT` pair (4b) —
   the "steer the next allocation through a global because the FFI has no
   out-parameter" shape, solved once as the slot's placement channel rather
   than three times. The channel is still implicit; what changed is that it
   is scoped to a (thread, device) instead of a thread.
