...
activation-backward kernels, the clamp adjoint, the RoPE `rotate_half` pair, the f32 fused FASE AdamW steps (single- and multi-tensor), and the SR-BF16 single-parameter step and rounding probe (`kernels.rs`) are described as KIR in `nsl_kir::kernels` and lowered
