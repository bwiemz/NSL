...
activation-backward kernels, the clamp adjoint, the RoPE `rotate_half` pair and the f32 fused FASE AdamW steps (single- and multi-tensor) (`kernels.rs`) are described as KIR in `nsl_kir::kernels` and lowered
