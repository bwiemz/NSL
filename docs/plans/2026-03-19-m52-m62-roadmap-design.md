...
- Weight constant folding: propagate known weight values through the computation graph
- Dead weight elimination: prune near-zero weights and their associated compute
- Sparsity-aware codegen: emit specialized kernels for weight matrices with known sparsity patterns
...
**Codegen changes:** New `WeightAwarePass` between semantic analysis and Cranelift emission. Loads safetensors into a `WeightMap`, annotates tensor operations with known values, runs constant propagation and dead code elimination on the tensor graph.
...
**Key components:**
- `nsl build --wcet` flag — enables WCET analysis pass
- Per-operation cycle counting: each tensor op maps to exact GPU/CPU cycle count (using M37's cost model)
- Memory access pattern analysis: prove no cache misses beyond a known worst case
- `@real_time(max_latency_ms=15.0)` decorator — compile error if WCET exceeds bound
- WCET certificate: JSON file with per-operation timing breakdown and proof chain
- DO-178C / ISO 26262 compliance: documentation generation for aerospace/automotive certification
- No heap allocation in certified path: compiler proves all memory is stack/slab-allocated

**Semantic changes:** New `WCETAnalyzer` pass that walks the static computation graph, summing worst-case cycle counts. Integrates with M37's GPU spec database for hardware-specific timing.
...
- Expression DAG → arithmetic circuit compiler (R1CS or AIR format)
- Integration with Halo2 (Zcash) and Plonky3 (Polygon) proving backends
...
**Codegen changes:** New `ZKCircuitEmitter` that translates the Wengert list (M40) into arithmetic circuit constraints. Fixed-point arithmetic for field-compatible computation.
...
- `agent.send(other_agent, kv_cache_ptr)` — zero-copy KV-cache transfer between agents
...
- Shared embedding table: multiple agents can borrow the same weight tensors (via `@shared`)
- `@pipeline_agent(agents=[drafter, reviewer, editor])` — declarative multi-agent pipeline
- Benchmarks: 1000x faster than LangChain/AutoGPT JSON serialization

**Semantic changes:** New `AgentChecker` that verifies ownership boundaries between agent scopes. Extends `OwnershipChecker` (M38a) with cross-agent transfer rules.
...
- Micro-checkpointing: async weight snapshots every N steps to shared storage (NVMe-oF or S3)
...
**Runtime additions:** `nsl_heartbeat_start`, `nsl_heartbeat_stop`, `nsl_elastic_resize`, `nsl_checkpoint_async`
...
- Topology-aware rank mapping: place TP groups within the same switch, PP stages across racks
...
**Codegen changes:** New `TopologyRouter` that reads cluster.json and emits optimized NCCL communicator initialization with topology-aware rank placement.
...
**Key components:**
- KIR → Verilog lowering: translate computation graph to hardware description language
- Systolic array mapping: partition matmul tiles into FPGA fabric
- `nsl build --target fpga --device xilinx-u280` — FPGA synthesis flow
- Groq LPU backend: KIR → Groq TSP assembly (linear execution model maps perfectly to KIR)
- Neuromorphic backend: KIR → SpiNNaker/Loihi spike-timing instructions
...
**Codegen changes:** New `backend_verilog.rs` and `backend_groq.rs` modules implementing KIR lowering to hardware-specific formats.
...
- RDMA data transfer: bypass OS kernel, NIC-to-GPU direct path via GPUDirect Storage
...
**Runtime additions:** `nsl_data_stream_init`, `nsl_data_stream_next_batch`, `nsl_data_stream_checkpoint`
...
| Feature | Why impossible in Python |
|---------|------------------------|
| M52 Weight-aware compilation | Requires AOT static graph + weight constants at compile time |
| M53 WCET proofs | Requires zero dynamic allocation + known instruction counts |
| M54 Unikernels | Requires no interpreter, no OS, no garbage collector |
| M55 ZK circuits | Requires static expression DAG, not dynamic Python loops |
| M56 Multi-agent safety | Requires linear types + effect system for memory safety proofs |
| M57 FPGA/neuromorphic | Requires static IR that maps to hardware description |
| M58 Elastic execution | Requires compiled SPMD with reshardable static graph |
| M59 Topology routing | Requires compile-time network topology awareness |
