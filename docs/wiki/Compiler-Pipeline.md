...
User `kernel` blocks have one path (roadmap A2 step 3 retired the direct AST→PTX `kernel.rs` the CUDA target used before): `KernelDef` AST nodes are lowered by [`kernel_lower.rs`](../../crates/nsl-codegen/src/kernel_lower.rs) (`lower_kernel_to_ir`) into the verified, backend-agnostic `KernelIR` — `let`, assignment to a declared local, element loads and stores, `if`/`elif`/`else`, `for ... in range(...)`, `while`, `break`/`continue`, a bare `return` and the index builtins; a local reassigned in a branch or a loop body is a block parameter — and then printed per target (see the dispatch in [`compiler/kernel.rs`](../../crates/nsl-codegen/src/compiler/kernel.rs) `compile_single_kernel`): [`backend_ptx.rs`](../../crates/nsl-kir/src/backend_ptx.rs) for the default `--target cuda`, `backend_amdgpu.rs` / `backend_metal.rs` / `backend_wgsl.rs` for `--target rocm|metal|webgpu` (those printers have no control flow yet, so a kernel whose KIR passes block arguments is refused there). Constructs outside the supported set are refused with a compile error pointing at the construct; there is no silent fallback.
