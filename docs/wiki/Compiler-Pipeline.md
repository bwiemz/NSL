...
2. **Portable subset (M47b, non-CUDA targets only)** — for `--target rocm|metal|webgpu`, `KernelDef` AST nodes are lowered by [`kernel_lower.rs`](../../crates/nsl-codegen/src/kernel_lower.rs) (`lower_kernel_to_ir`) into a backend-agnostic `KernelIR`, then emitted by the per-backend lowerers (`backend_amdgpu.rs`, `backend_metal.rs`, `backend_wgsl.rs`; [`backend_ptx.rs`](../../crates/nsl-kir/src/backend_ptx.rs) exists for KIR→PTX but is not in the CUDA dispatch). Stores and control flow on this path are deferred to M47c and refuse with a compile error; there is no automatic fallback between the two paths.
