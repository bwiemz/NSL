...
- **AWQ retention -- subprocess gaps** -- arena ordering fix shipped (PR #98); two structural gaps remain: subprocess does not call `model_forward`, and the model library is not linked into the calibration binary. Blocking WGGO Phase 2.
- **WGGO Phase 2 gradient scoring** -- trait, scorers, sidecar schema, and hook shipped; real backward-pass execution blocked on AWQ retention subprocess fix above.
...
- **WRGA B.3.2 fused backward** -- per-op profiling benchmark shipped (commit `ab3ef5d9`); scope analysis in progress (backward appears allocator/tape-dominated; fused-backward speedup estimate revised to 1.2-1.5x). Design: `docs/plans/2026-04-18-wrga-b32-fused-backward-STUB.md`.
