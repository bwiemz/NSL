...
- **WRGA B.4 fused-forward staging** -- *next WRGA milestone (surfaced 2026-05-23)*. The per-op breakdown shows the fused GatedLoRA **forward** kernel is **73.4% of GPU time** (~4.2s/iter for one launch) -- the single-threaded thread-0-only staging pathology (all four `emit_lora_stage_{x,w,a,b}_tile` helpers gate global->SMEM loads on `%tid_x == 0`). Parallelizing staging across the 32 warp lanes is the actual optimization win. Pragmatic stopgap: the fused forward is ~9-10x slower than the unfused cuBLAS path at training scale, so gating it off for large-m is a net win on its own. Scope: `docs/plans/2026-05-23-wrga-b4-fused-forward-staging-scope.md`.
