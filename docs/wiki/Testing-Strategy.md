...
| `nsl-codegen` | `compile` | `nsl_codegen::compile` (the `nsl build` single-file entry) over the clean import-free examples |
...
`@autotune` kernel, a CFIE `serve` block — the last two print per compile)
and says so on stderr — so compare the per-byte or per-token figure, which
survives the file set drifting, over the absolute time. The `compile` bench
measures the single-file entry only; a recipe graph needs the CLI's module
loader and import tables, which live inside `run_build_multi` (roadmap A1).
...
apart ("Performance has regressed", full 100-sample runs) on this project's
own workstation while a training job shared the box.
