...
- One fresh Stack v3 shard (index **204**, same pinned revision as train)
  ...
  provenance. `fetch.py --only stack-val` refuses any shard already in the
  manifest's recorded TRAIN file list (a stride formula would go stale the
  moment `--stack-shards` changed), and the corpus gate pins the two
  committed file lists disjoint.
...
- Disjointness is EVIDENCE, not an argument: Stack v3 rows carry a
  ...
  requires an empty overlap — and requires BOTH sides non-empty, so a
  cleared train dir cannot pass vacuously. The check runs over the WHOLE
  val shard (21,202 repos ∩ 421,948 = 0), stronger than the 1,773-repo
  extraction it feeds.
...
(full + sanitized; text/prompt + code + test_list + challenge_test_list +
test_setup_code — the challenge tests some harnesses execute were a review
catch) — 7,269 distinct reference lines
...
| **val-sft** | 2,858 | **0** | re-cut: the first scan found **14 genuine benchmark problems (7 HumanEval, 7 MBPP** — attribution verified by disjoint per-benchmark line sets in review**)** in the distillation validation split; `extract.py --drop-contaminated` removed them |
...
Scan cost: 73 GB in ~13 min per full pass (two passes ran: the train numbers were identical under the review-expanded reference set). The dropping filter and the verifying scan
...
- `verify.py`: train/val head+tail disjointness generalized to ALL pairs
  ...
  intersection, and the decontamination record — 47/47 checks.
