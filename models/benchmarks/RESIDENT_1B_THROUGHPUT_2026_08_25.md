...
Method: every arm built once (`nsl build`), 3 timing rounds interleaved
round-robin (best round reported; the PR #454 cold-clock lesson), 30
optimizer steps per run with the first 10 excluded, tok/s from steady-state
`OPT_STEP` arrival stamps (cadence-independent), NSL_EVENTS exact-byte
memory series per step, and SEPARATE instrumented builds/runs for the
phase split, PCIe counters and kernel trace — instrumentation is never
timed. RTX PRO 4500 Blackwell 32 GB, tf32 roofline 87.6 TFLOPS (measured),
`Scale("1b").flops_per_token(2048)`, synthetic 491,520-token stream.

## The headline: the resident path is ~30% faster

| arm | tok/s (best) | MFU | alloc peak | activations | optim on dev | PCIe h2d/d2h (30 steps) | fwd/bwd/opt (s) |
|---|---|---|---|---|---|---|---|
...
| resident B8/A1 | **OOM** | — | 12.02 GiB allocated at failure | — | — | — | — |
| offload B8/A1 | **OOM** | — | 4.02 GiB allocated at failure | — | — | — | — |
...
- **Where the 30% lives**: the phase split puts the offload optimizer phase
  ...
  IDENTICAL (top: `flash_attn_bwd_main` 0.79 s, `sgemm_cublas` 0.67 s/574
  launches) — the compute stream does not differ; the delta is entirely
  host↔device optimizer traffic. PCIe counters agree: 16.4 GiB h2d +
  12.3 GiB d2h per 30-update run offloaded, ~1 MiB resident.
...
- **B4/A2 is slightly SLOWER than B2/A4** on the resident path (2,827 vs
  ...
  micro-batches here.
- **B8/A1 OOMs in BOTH postures**, resident and offload, on the same
  request: the MLP intermediate at B8 rows — 16,384 × 8,192 × 4 B =
  exactly the 512.0 MB the abort names — against a driver-exhausted card
  (237–261 MB free of 31.39 GB) while ALLOCATED bytes were only 12.02 /
  4.02 GiB: transient-pool reservation growth at B8-sized surfaces, not
  allocated tensors, exhausts the card. B8/A1@2048 is out of reach on
  32 GB in this posture, offload included. (A1 also cannot run any
  `--layerwise-accum` arm — accumulation ≥ 2 is required there — so no
  layerwise escape hatch exists either.)
- Allocation series were FLAT for every completing arm (exact-byte
  NSL_EVENTS post-cleanup series) — corroborating the slope-gate design
  that item 2 ships.
...
the resident run does not need (19.47 GiB peak of 31.39 usable leaves
~11.9 GiB of margin). **The resident configuration should become the 1B
