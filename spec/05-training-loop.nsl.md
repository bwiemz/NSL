...
- `optimizer:` / `scheduler:` / `data:` / `step()` / `callbacks:` behave
  exactly as in `train`. `distribute:` is not supported, as on `train`, which
  refuses the same section; distribution is configured through CLI options.
  (`@pipeline` is the remedy `train` offers, but a `distill` block refuses
  pipeline-parallel training outright in CPKD v1, so it is not one here.
  This line used to attribute the refusal to a deferred CPDT composition.
  CPDT × `distill` is NOT deferred: `--cpdt full --cpdt-num-gpus N` plans
  weights-only for a `distill` block and lowers the student's
...
- A `distill` block is a training loop, and the module-level analyses that
  ...
  and a packed `data:` corpus selects the segment-masked attention kernels
  exactly as it does under `train`. Not yet universal — `nsl profile` still
  resolves only `train(...)` and reports "no train block or top-level fn
  found" for a distill-only module.
